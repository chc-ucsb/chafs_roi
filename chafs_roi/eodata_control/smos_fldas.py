...
def ExtractAdmSMOS_fldas(year, fnid_dict):
    # Get filenames
    fn_data = '/home/chc-sandbox/people/donghoonlee/fldas/africa/SoilMoi00_10cm_tavg_{:04d}??.nc'
    ...
    filn_out_crop = '/home/donghoonlee/chafs/data/eodata/smos_fldas/adm.smos.fldas.crop.{:04d}.{:02d}.{:02d}.hdf'
    filn_out_all = '/home/donghoonlee/chafs/data/eodata/smos_fldas/adm.smos.fldas.all.{:04d}.{:02d}.{:02d}.hdf'
    ...
    fn_sample ='/home/chc-sandbox/people/donghoonlee/fldas/africa/SoilMoi00_10cm_tavg_198201.nc'
    ...
    data = data.variables['SoilMoi00_10cm_tavg'].values
...
def smos_fldas():
    ...
    fn_rdx_out = '/home/donghoonlee/chafs/data/eodata/rdx.adm.smos.fldas.pickle'
    if False:
        fn_sample ='/home/chc-sandbox/people/donghoonlee/fldas/africa/SoilMoi00_10cm_tavg_198201.nc'
        data_sample = xr.open_dataset(fn_sample)
        rdx_reduced = RasterizeAdminIndex(shape_adm, 'ID', data_sample['SoilMoi00_10cm_tavg'])
...
#     files_rm = sorted(glob.glob('/home/donghoonlee/chafs/data/eodata/smos_fldas/adm.smos.fldas.crop.????.??.??.hdf'))
#     if len(files_rm) > 0: 
...
#             os.remove(file)
    ...
    with Pool(processes=8) as pool:
        pool.starmap(ExtractAdmSMOS_fldas, args)
        pool.close()
        pool.join()
