...
def ExtractAdmNDVI_eviirs(year, fnid_dict):
    ...
    filn_out_crop = '/home/donghoonlee/chafs/data/eodata/ndvi_eviirs/adm.ndvi.eviirs.crop.{:04d}.{:02d}.{:02d}.hdf'
    filn_out_all = '/home/donghoonlee/chafs/data/eodata/ndvi_eviirs/adm.ndvi.eviirs.all.{:04d}.{:02d}.{:02d}.hdf'
    ...
    with open('/home/donghoonlee/chafs/data/eodata/rdxu.adm.ndvi.eviirs.pickle', 'rb') as f:
        rdx_unique = cPickle.load(f)    
    with open('/home/donghoonlee/chafs/data/eodata/rdxr.adm.ndvi.eviirs.pickle', 'rb') as f:
...
def ndvi_eviirs():
    ...
    fn_rdx_unique = '/home/donghoonlee/chafs/data/eodata/rdxu.adm.ndvi.eviirs.pickle'
    fn_rdx_reduced = '/home/donghoonlee/chafs/data/eodata/rdxr.adm.ndvi.eviirs.pickle'
    if False:
        fn_sample = '/home/chc-sandbox/people/dlee/ndvi_eviirs/ndvi.eviirs.2012.021.tif'
    ...
    files_rm = sorted(glob.glob('/home/donghoonlee/chafs/data/eodata/ndvi_eviirs/adm.ndvi.eviirs.crop.????.*.hdf'))[-5:]
    if len(files_rm) > 0: 
        for file in files_rm: 
            os.remove(file)

    stime = time.time()
    args = ((year, fnid_dict) for year in np.arange(2012, 2024))
