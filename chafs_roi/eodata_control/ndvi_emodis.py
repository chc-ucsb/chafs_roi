...
def ExtractAdmNDVI_emodis(year, fnid_dict):
    # Get filenames
    fn_data = '/home/chc-sandbox/people/dlee/ndvi_emodis/ndvi.emodis.{:04d}*.tif'
...
def ndvi_emodis():
    ...
    # stime = time.time()
    # ExtractAdmNDVI_year(2002, fnid_dict)
    # print(time.time() - stime)

    stime = time.time()
    args = ((year, fnid_dict) for year in np.arange(2002, 2023))
    with Pool(processes=8) as pool:
        pool.starmap(ExtractAdmNDVI_emodis, args)
        pool.close()
        pool.join()
    print(time.time() - stime)
