...
def ExtractAdmPRCP_chirps_prelim(year, fnid_dict):
    
    print(year)
    
    # Get filenames
    fn_data = '/home/chc-data-out/products/CHIRPS-2.0/prelim/global_daily/netcdf/p05/chirps-v2.0.{:04d}.days_p05.nc'
    ...
    filn_out_crop = '/home/donghoonlee/chafs/data/eodata/prcp_chirps-v2p/adm.prcp.chirps-v2p.crop.{:04d}.{:02d}.{:02d}.hdf'
    filn_out_all = '/home/donghoonlee/chafs/data/eodata/prcp_chirps-v2p/adm.prcp.chirps-v2p.all.{:04d}.{:02d}.{:02d}.hdf'
...
def prcp_chirps_prelim():
    ...
    if False:
        ...
        rdx_reduced = RasterizeAdminIndex(shape_adm, 'ID', data_sample['precip'])
    ...
    # Running
    stime = time.time()
    # args = ((year, fnid_dict) for year in np.arange(2023, 2024))
    # with Pool(processes=8) as pool:
    #     pool.starmap(ExtractAdmPRCP_chirps_prelim, args)
    #     pool.close()
    #     pool.join()
    
    ExtractAdmPRCP_chirps_prelim(2023, fnid_dict)
