...
def ExtractAdmTMAX_noaa(year, fnid_dict):
    # Get filenames
    fn_data = '/home/chc-sandbox/people/dlee/temp_noaa-cpc/tmax.noaa-cpc.{:04d}*.nc'
    ...
    filn_out_crop = '/home/donghoonlee/chafs/data/eodata/tmax_noaa-cpc/adm.tmax.noaa-cpc.crop.{:04d}.{:02d}.{:02d}.hdf'
    filn_out_all = '/home/donghoonlee/chafs/data/eodata/tmax_noaa-cpc/adm.tmax.noaa-cpc.all.{:04d}.{:02d}.{:02d}.hdf'
    ...
    fn_sample = '/home/chc-sandbox/people/dlee/temp_noaa-cpc/tmax.noaa-cpc.1979.01.01.nc'
    cropland = RasterResampling(fn_cropland, fn_sample).flatten()

    # Load the reduced raster indicies
    with open('/home/donghoonlee/chafs/data/eodata/rdx.adm.tmax.noaa-cpc.pickle', 'rb') as f:
    ...
    data = data.variables['tmax'].values
...
def tmax_noaa_cpc():
    ...
    fn_rdx_out = '/home/donghoonlee/chafs/data/eodata/rdx.adm.tmax.noaa-cpc.pickle'
    if False:
        fn_sample = '/home/chc-sandbox/people/dlee/temp_noaa-cpc/tmax.noaa-cpc.1979.01.01.nc'
        data_sample = xr.open_dataset(fn_sample)
        rdx_reduced = RasterizeAdminIndex(shape_adm, 'ID', data_sample['tmax'])
    ...
    files_rm = sorted(glob.glob('/home/donghoonlee/chafs/data/eodata/tmax_noaa-cpc/adm.tmax.noaa-cpc.crop.????.*.hdf'))[-45:]
    if len(files_rm) > 0: 
    ...
    # Running
    stime = time.time()
    args = ((year, fnid_dict) for year in np.arange(1979, 2024))
    with Pool(processes=8) as pool:
        pool.starmap(ExtractAdmTMAX_noaa, args)
        pool.close()
        pool.join()
